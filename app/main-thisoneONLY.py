...
def detect_assumptions(payload: AssessmentInput) -> list[str]:
    ...
    if payload.financial_uploads_present and payload.financial_metrics:
        assumptions.append("Uploaded financial statements were parsed into structured financial metrics.")
...
def rows_from_pdf_text_lines(text: str) -> list[list[Any]]:
    ...
    for raw_line in text.splitlines():
        ...
        if not cleaned:
            continue
        parsed = split_pdf_label_and_values(cleaned)
...
def pick_latest_number(row: list[Any]) -> Optional[float]:
    numbers = [parse_money_or_number(cell) for cell in row[1:]]
    numbers = [
        n for n in numbers
...
FINANCIAL_LABEL_ALIASES: dict[str, list[str]] = {
    "revenue": ["revenue", "sales", "total revenue", "service revenue"],
    "cogs": ["cost of goods sold", "cogs", "cost of sales", "direct costs"],
    "gross_profit": ["gross profit"],
    "operating_expenses": ["operating expenses", "opex", "total expenses"],
    "operating_income": ["operating income", "ebit"],
    "interest_expense": ["interest expense"],
    "net_income": ["net income", "profit", "net profit"],
    "cash": ["cash", "cash equivalents"],
    "accounts_receivable": ["accounts receivable", "ar", "a r"],
    "inventory": ["inventory"],
    "total_assets": ["total assets"],
    "accounts_payable": ["accounts payable", "ap", "a p"],
    "debt": ["debt", "loans", "notes payable"],
    "owner_equity": ["owner equity", "equity", "retained earnings"],
    "net_cash_from_operations": ["net cash from operations", "operating cash flow", "cash from operations"],
    "capital_expenditures": ["capital expenditures", "capex"],
    "debt_repayment": ["debt repayment", "loan repayment", "principal repayment"],
    "net_change_in_cash": ["net change in cash", "change in cash"],
}
...
def extract_metrics_from_rows(rows: list[list[Any]]) -> dict[str, float]:
    metrics: dict[str, float] = {}
    for row in rows:
        if not row:
            continue
        key = metric_key_from_label(str(row[0]))
        if not key:
            continue
        value = pick_latest_number(row)
...
def parse_financial_uploads(uploads: dict[str, Optional[dict[str, str]]]) -> dict[str, Any]:
    parsed: dict[str, Any] = {"statements": {}, "combined_metrics": {}, "parse_errors": []}
    for statement_type, metadata in uploads.items():
        ...
        try:
            ...
            parsed["statements"][statement_type] = {
...
def apply_parsed_financial_metrics(payload_dict: dict[str, Any], parsed_financials: dict[str, Any]) -> dict[str, Any]:
    ...
    payload_dict["financial_parse_summary"] = (
        f"Extracted {len(extracted)} financial metric(s): {', '.join(extracted[:12])}"
        if extracted
        else "Financial files were uploaded, but no structured metrics were extracted."
    )
    if errors:
...
def health() -> dict[str, Any]:
    return {
        ...
        "financial_upload_phase": "phase_3_parse_structured_metrics",
    }
