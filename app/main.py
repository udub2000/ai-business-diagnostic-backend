...
CONSULTANT_BRIEF_SYSTEM_PROMPT = """You are preparing an INTERNAL consultant preparation brief for The Uniikye Truth.
The brief is not client-facing and is not a replacement for the consultant's judgment.

Your job is to help the consultant prepare for a discovery/diagnostic conversation using only the supplied intake,
scoring results, parsed financial information, contradictions, assumptions, and client-facing report.

Rules:
- Separate observed/submitted facts from hypotheses.
- Do not invent facts, motives, causes, or financial conclusions that are not supported.
- Treat possible root causes as hypotheses to test, not conclusions.
- Explicitly identify missing information that could materially change the diagnosis.
- Surface contradictions and weak evidence instead of smoothing them over.
- Use the client's notes when they contain useful context.
- Financial observations should identify what deserves investigation, not provide tax, legal, or investment advice.
- Questions should be specific enough to use in a live consultation.
- Do not simply repeat the client-facing report.
- Do not write a sales pitch.
- Do not use tables or JSON.
- Keep the exact section headings requested by the user prompt.
"""


def fallback_consultant_brief_markdown(
    payload: AssessmentInput,
    result: Dict[str, Any],
    submitted_payload: Dict[str, Any],
    reason: str = "",
) -> str:
    primary = result.get("primary_constraint") or "Not determined"
    secondary = result.get("secondary_constraint") or "None identified"
    confidence = f"{result.get('overall_confidence_label', 'N/A')} ({result.get('overall_confidence_score', 'N/A')}/100)"
    assumptions = result.get("assumptions") or []
    contradictions = result.get("contradictions") or []
    notes = submitted_payload.get("notes") or "No narrative notes were provided."
    missing = [k for k in (
        "leads_per_week","conversion_rate","backlog_days","jobs_per_crew_per_day",
        "utilization_rate","monthly_revenue","labor_cost_pct","gross_margin_pct"
    ) if submitted_payload.get(k) in (None, "")]
    missing_text = ", ".join(x.replace("_", " ").title() for x in missing) if missing else "No major core metric gaps detected from the structured intake."

    return f"""# Consultant Brief - INTERNAL

## Situation at a Glance
Primary diagnostic signal: **{primary}**. Secondary signal: **{secondary}**. Confidence: **{confidence}**.
Client notes: {notes}

## What the Diagnostic Thinks Is Happening
The current scoring and report logic point to **{primary}**. Treat this as the working diagnosis to test in conversation, not as a final conclusion.

## Evidence Supporting That Diagnosis
- Primary constraint returned by the diagnostic: {primary}
- Secondary constraint returned by the diagnostic: {secondary}
- Constraint pattern: {result.get('constraint_pattern') or 'Not provided'}
- Strategic state: {result.get('strategic_state') or 'Not provided'}

## What I Would Not Assume Yet
Do not assume the primary constraint is the root cause until the operating workflow, customer behavior, and financial context are discussed with the owner.

## Contradictions / Data That Doesn't Fit
{chr(10).join('- ' + str(x) for x in contradictions) if contradictions else '- No contradictions were flagged by the current scoring model.'}

## Missing Information That Matters
- {missing_text}

## Financial Items Worth Investigating
Review the uploaded statements and parsed financial metrics against the owner's explanation of pricing, labor, cash timing, overhead, and seasonality. Confirm any material figure before relying on it in the consultation.

## Questions to Ask the Owner
- What changed most in the business during the last 6-12 months?
- Where does work most often slow down, get delayed, or require owner intervention?
- Which part of the current diagnosis feels most accurate to you, and which part does not?
- What have you already tried to fix this issue, and what happened?
- Which numbers do you trust most, and which are estimates?
- What would have to be true for the primary diagnosis to be wrong?

## Areas to Observe During the Consultation
- Whether the owner's explanation matches the submitted metrics
- Repeated handoffs, delays, rework, or owner-dependent decisions
- Gaps between stated process and actual process
- Whether financial pressure is a cause, symptom, or separate issue

## Potential Root Causes to Test
- Process inconsistency or undocumented workflow
- Capacity allocation or scheduling mismatch
- Pricing/margin pressure
- Lead handling or conversion friction
- Data-quality or measurement gaps

## Possible Opportunities Beyond the Primary Constraint
Explore secondary improvements only after testing the primary diagnosis and understanding dependencies between demand, conversion, capacity, operations, and financial performance.

## Recommended Consultation Agenda
1. Confirm the owner's goals and what prompted the diagnostic.
2. Validate the submitted facts and clarify estimates.
3. Test the primary diagnosis against the owner's lived experience.
4. Investigate contradictions, missing information, and financial questions.
5. Identify the most plausible root causes.
6. Agree on what should be measured or investigated next before prescribing a larger solution.

_Internal fallback used: {_pdf_safe_text(reason) if reason else 'Claude brief unavailable'}_
""".strip()
...
def build_consultant_brief_markdown(
    ...
    api_key = os.getenv("ANTHROPIC_API_KEY")
    if not api_key:
        return fallback_consultant_brief_markdown(payload, result, submitted_payload, "Missing ANTHROPIC_API_KEY")

    model = os.getenv("ANTHROPIC_MODEL", "claude-sonnet-4-6")
    client = anthropic.Anthropic(api_key=api_key)

    evidence = {
        "submitted_intake": submitted_payload,
        "diagnostic_result": result,
        "client_facing_report": report_markdown,
    }
    prompt = """Create the internal Consultant Brief from the evidence below.
...
The purpose is consultation preparation. Do not solve the entire business on paper.
For each hypothesis, make clear what evidence supports it and what the consultant should validate.
Prioritize questions that could change the diagnosis or materially change the recommended next step.
...
""" + json.dumps(evidence, default=str, ensure_ascii=False, indent=2)
    ...
    for attempt in range(1, 5):
        try:
            print(f"=== CONSULTANT BRIEF CLAUDE CALL START attempt {attempt}/4 ===", flush=True)
            with client.messages.stream(
                model=model,
                max_tokens=3500,
            ...
            brief = "\n\n".join(parts).strip()
            if len(brief) >= 900 and "## Questions to Ask the Owner" in brief and "## Recommended Consultation Agenda" in brief:
                print(f"=== CONSULTANT BRIEF CREATED: {len(brief)} chars ===", flush=True)
                return brief
            last_error = f"Incomplete consultant brief on attempt {attempt}"
        except Exception as exc:
            overloaded = is_claude_overloaded_error(exc)
    ...
    return fallback_consultant_brief_markdown(payload, result, submitted_payload, last_error)
