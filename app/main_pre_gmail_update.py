...
GMAIL_SMTP_USER = os.getenv("GMAIL_SMTP_USER", DIAGNOSTIC_NOTIFICATION_EMAIL)
...
def send_diagnostic_email(
    ...
    report_markdown: str,
    assessment_id: str,
    uploads: Optional[dict[str, Optional[dict[str, str]]]] = None,
) -> None:
    """Email a completed diagnostic to The Uniikye Truth business inbox."""
    if not GMAIL_SMTP_USER or not GMAIL_APP_PASSWORD:
        raise RuntimeError("Diagnostic email is not configured. Set GMAIL_SMTP_USER and GMAIL_APP_PASSWORD.")
...
FULL BUSINESS DIAGNOSTIC REPORT

{report_markdown}
"""
    msg.set_content(body)

    for metadata in (uploads or {}).values():
        if not metadata or not metadata.get("file_path"):
            continue
        path = Path(metadata["file_path"])
        if not path.exists():
            continue
        ...
        msg.add_attachment(
            ...
            maintype=maintype,
            subtype=subtype,
            filename=metadata.get("file_name") or path.name,
        )

    print("=== DIAGNOSTIC EMAIL START: smtp.gmail.com:587 STARTTLS ===", flush=True)
    ...
    try:
        with smtplib.SMTP("smtp.gmail.com", 587, timeout=30) as smtp:
            ...
            smtp.ehlo()
            print("=== DIAGNOSTIC EMAIL TLS CONNECTED ===", flush=True)
            smtp.login(GMAIL_SMTP_USER, GMAIL_APP_PASSWORD)
            print("=== DIAGNOSTIC EMAIL AUTHENTICATED ===", flush=True)
            smtp.send_message(msg)
            print(f"=== DIAGNOSTIC EMAIL SENT to {DIAGNOSTIC_NOTIFICATION_EMAIL} ===", flush=True)
    except Exception as exc:
